...
export default (state = defaultState,action)=>{
	...
	if(action.type == types.SET_DETAIL){//处理输入时数据变化
		 return state.merge({
		 	'name':fromJS(action.payload.name),
		 	'wechat':fromJS(action.payload.wechat),
		 	'opinionImages':fromJS(action.payload.opinionImages),
		 	'opinion':fromJS(action.payload.opinion)
		 })
	}
