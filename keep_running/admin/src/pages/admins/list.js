...
class AdminList extends Component{//自定义组件名字首字母都要大写，而html组件则就是个一个html标签
	constructor(props){
		super(props)
	}
	componentDidMount(){
	...
	render(){//render负责渲染页面
		const columns = [
		 ...
		 const dataSource = list.map((admin)=>{
		 	return{
		 		key:admin.get('id'),
		 		name:admin.get('name'),
		 		email:admin.get('email'),
		 		phone:admin.get('phone'),
		 		createdAt:moment(admin.get('create_time')).format('YYYY-MM-DD HH:MM:SS')
		 	}
		 }).toJS()
		return(
 			<div className = 'AdminList'>
 				<AdminLayout>
	                ...
	                <div className='btn'>
			        	<Link to='/admin/add'><Button type="primary" className='add-btn'>新增管理员</Button></Link>
			        </div>
...
const mapStateToProps = (state) =>{	
	return{
		list:state.get('admin').get('list'),
		current:state.get('admin').get('current'),
		pageSize:state.get('admin').get('pageSize'),
		total:state.get('admin').get('total'),
...
const mapDispatchToProps =(dispatch)=>{//利用接收的dispatch参数，进行派发action
	return{//将方法都需要返回一个对象，
		...
		handleDel:(id)=>{
			dispatch(actionCreator.delAdminAction(id))
		}	
	}
}
